...
parser$add_argument('--latitude-lower', type = 'double')
parser$add_argument('--latitude-upper', type = 'double')
...
perturbations_region <- perturbations_split %>%
  filter(
    latitude_bottom >= args$latitude_lower,
    latitude_bottom < args$latitude_upper
  ) %>%
...
output <- wrap_plots(lapply(sort(unique(emissions$inventory)), function(inventory_i) {
  ...
  theme(
    ...
    legend.position = if (args$in_supplement) {
      'right'
    } else {
      'bottom'
    },
...
output <- output +
  plot_annotation(
    ...
    theme = theme(
      plot.title = element_text(
        hjust = if (args$in_supplement) 0.32 else 0.5,
        size = 13
      )
    )
  )
...
ggsave_base(
  ...
  height = if (args$in_supplement) 11.7 else 13
)
